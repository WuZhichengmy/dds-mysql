...
public class LocalStorageController {
    ...
    public ResponseEntity<LocalStorage> create(@RequestParam("file") MultipartFile file) {
        return new ResponseEntity<>(localStorageService.create(file), HttpStatus.CREATED);
    }

    @ApiOperation("下载文件")
    @GetMapping("/download/{id}")
    public void downloadAttachment(@PathVariable Long id, HttpServletRequest request, HttpServletResponse response) {
        localStorageService.downloadAttachment(id, request, response);
    }
    ...
    public ResponseEntity<Object> upload(@RequestParam MultipartFile file){
        ...
        LocalStorage localStorage = localStorageService.create(file);
